...
## Apply OTA via adb
    ...
    $ adb sideload dist_output/aosp_quartz-ota-eng.${USER}.zip
    $ adb reboot
...
2.  Generate json config with:
    ...
    ```
     ...
     $ PYTHONPATH=$ANDROID_BUILD_TOP/build/make/tools/releasetools:$PYTHONPATH \
         ...
         dist_output/aosp_quartz-ota-eng.${USER}.zip \
         dist_output/aosp_quartz-ota-eng.${USER}.json \
         file:///data/user/0/com.example.android.systemupdatersample/files/packages/aosp_quartz-ota-eng.${USER}.zip
    ```
...
3.  Push the files on the board:
    ...
    ```
     ...
     $ adb push dist_output/aosp_quartz-ota-eng.${USER}.json /data/user/0/com.example.android.systemupdatersample/files/configs/
     $ adb push dist_output/aosp_quartz-ota-eng.${USER}.zip /data/user/0/com.example.android.systemupdatersample/files/packages/
    ```
